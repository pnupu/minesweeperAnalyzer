...
// Message function for extension environment
...
// Initialize binomial cache and related objects
let binomialCache = null;
let BINOMIAL = null;

// Initialize binomial objects when needed
function initializeBinomial() {
    if (!binomialCache && typeof Binomial !== 'undefined' && typeof BinomialCache !== 'undefined') {
        BINOMIAL = new Binomial(1000, true);
        binomialCache = new BinomialCache(100, 50, BINOMIAL);
        console.log('Binomial cache initialized');
    }
}
...
async function solver(board, options) {
    ...
    // Initialize binomial cache if not already done
    initializeBinomial();
    ...
    async function doSolve(board, options) {
        ...
        const pe = new ProbabilityEngine(board, witnesses, witnessed, squaresLeft, Math.max(0, minesLeft), options);
    ...
    function secondarySafetyAnalysis(pe, board, action, best, ltr) {
        ...
        if (dominated) {
            ...
            tile.setProbability(action.prob, action.progress);  // a dominated tile has 100% progress
    ...
    function runProbabilityEngine(board, notMines) {
        ...
        localOptions.playStyle = PLAY_STYLE_EFFICIENCY;  // this forces the pe to do a complete run even if local clears are found

        const effectiveMinesLeftForRunPE = Math.max(0, minesLeft);
        const pe = new ProbabilityEngine(board, witnesses, witnessed, squaresLeft, effectiveMinesLeftForRunPE, localOptions);
...
// Make solver function and classes available as globals
if (typeof window !== 'undefined') {
  window.solver = solver;
  window.SolverGlobal = SolverGlobal;
  window.Action = Action;
  window.Filling = Filling;
}
